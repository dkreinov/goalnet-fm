...
def build_playerval(mids):
    ...
    cval = {}                       # (club_id, season_label) -> squad value
    cval_any = {}                   # club_id -> latest squad value (fallback)
    ...
    pclub = {}                      # player_id -> most-recent snapshot club_id (real club, for national fallback)
    for pid, cid in con.execute("SELECT player_id,club_id FROM player_snapshot WHERE club_id IS NOT NULL ORDER BY snapshot_date"):
        pclub[pid] = cid
    starters = defaultdict(list)    # match_id -> [(player_id, club_id_in_match)]
    ...
    def pval(pid, teamcid, lab):
        ...
        if v: return v
        rc = pclub.get(pid)                                     # national: player's real club
        if rc is not None:
            return cval.get((rc, lab)) or cval_any.get(rc)
        return None
    ...
    for m in mids:
        ...
        for pid, cid in starters.get(m, []):
            v = pval(pid, cid, lab)
            (hv if cid == hc else av).append(v)
        hvv = [x for x in hv if x]; avv = [x for x in av if x]
        hmean = lv(np.mean(hvv)) if hvv else 0.0; amean = lv(np.mean(avv)) if avv else 0.0
...
def main():
    ...
    print("building per-player club-value features...", flush=True)
    ...
    print(f"player-value coverage: ALL both-sides {float(((PV[:,3]>0.5)&(PV[:,4]>0.5)).mean())*100:.0f}% | NATIONAL {covn*100:.0f}%", flush=True)
    ...
    print("=== FREE per-player club-value pre-test (held-out test) ===", flush=True)
    run(CTX0, "base ctx(10)")
    run(np.concatenate([CTX0, PV], 1), "+player_value(15)")
