"""Market-value ablation (Study 1, CLUB lane = the gate). Does transfermarkt squad value add signal to GoalNet
beyond FM attrs + Elo/form context? Value features are joined from club_season_tm by (club_id, season). For
national matches there's no club value (national teams aren't clubs) -> those rows are 'missing' here, so this
study measures value's effect on the CLUB-heavy ALL test. If value helps clubs, collect national squad values
and run Study 2. Single-seed train-split A/B on held-out test (ALL + national subset).
Usage: python experiments/value_ablation.py [--epochs 60]
"""
...
def build_value(mids):
    """Per-match value features: [log_home_val, log_away_val, log_val_diff, has_home, has_away, age_diff,
    size_diff] from club_season_tm (squad + top11 value, avg_age, squad_size). Missing -> 0 + flag."""
    con = db.connect()
    ...
    val = {}
    for cid, season, sv, t11, age, size in con.execute(
            "SELECT club_id,season,squad_value_eur,top11_value_eur,avg_age,squad_size FROM club_season_tm"):
        val[(cid, season)] = (sv, t11, age, size)
    con.close()
    def lv(x): return math.log1p(x) if x else 0.0
    SQ, T11, OTH = [], [], []
    for m in mids:
        si, hc, ac = minfo.get(m, (None, None, None)); lab = seasonlab.get(si)
        hv = val.get((hc, lab)); av = val.get((ac, lab))
        hsv = hv[0] if hv else None; asv = av[0] if av else None
        ht11 = hv[1] if hv else None; at11 = av[1] if av else None
        hh, ha = float(hv is not None), float(av is not None)
        sqdiff = (lv(hsv) - lv(asv)) if (hsv and asv) else 0.0
        t11diff = (lv(ht11) - lv(at11)) if (ht11 and at11) else 0.0
        agediff = ((hv[2] or 0) - (av[2] or 0)) if (hv and av) else 0.0
        sizediff = ((hv[3] or 0) - (av[3] or 0)) if (hv and av) else 0.0
        SQ.append([lv(hsv), lv(asv), sqdiff, hh, ha])
        T11.append([lv(ht11), lv(at11), t11diff])
        OTH.append([agediff, sizediff])
    return np.array(SQ, np.float32), np.array(T11, np.float32), np.array(OTH, np.float32)
...
def main():
    ...
    print("building value features from club_season_tm...", flush=True)
    SQ, T11, OTH = build_value(mids)
    cov = float((SQ[:, 3] * SQ[:, 4]).mean())   # both sides valued
    print(f"value coverage (both clubs valued): {cov*100:.0f}% of {len(mids)} matches", flush=True)
    con = db.connect()
    ...
    print("=== Study 1: market-value ablation (CLUB gate, held-out test) ===", flush=True)
    run(CTX0, "base ctx(10)")
    run(np.concatenate([CTX0, SQ], 1), "+squad_value(15)")
    run(np.concatenate([CTX0, T11], 1), "+top11_value(13)")
    run(np.concatenate([CTX0, SQ, T11, OTH], 1), "+all_value(20)")
